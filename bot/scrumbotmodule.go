...
func (b *Bot) restartScrum(event *slack.MessageEvent) bool {
	...
	if !b.scrum.DeleteLastReport(user.Name) {
		_, _, _ = b.slackBotAPI.PostMessage(event.Channel, slack.MsgOptionText("Nothing to restart, let's get out.", false), AsUser)
		return false
	}

	_, _, _ = b.slackBotAPI.PostMessage(event.Channel, slack.MsgOptionText("Your last report was deleted, you can `start scrum` a new one again.", false), AsUser)
	return false
}
...
func (b *Bot) startScrum(event *slack.MessageEvent, isSkipped bool) bool {
	...
	if err != nil {
		b.logSlackRelatedError(event, err, "Fail to get user information.")
		_, _, _ = b.slackBotAPI.PostMessage(event.Channel, slack.MsgOptionText("There was an error starting your scrum, please try again.", false), AsUser)
		return false
	}
	username := user.Name
	...
	if len(teams) == 0 {
		_, _, _ = b.slackBotAPI.PostMessage(event.Channel, slack.MsgOptionText("You're not part of a team, no point in doing a scrum report.", false), AsUser)
	}

	if len(teams) == 1 {
		return b.chosenTeam(event, username, teams[0], isSkipped)
	}
...
func (b *Bot) chooseTeam(event *slack.MessageEvent, username string, teams []string, isSkipped bool) bool {
	...
	b.setUserContext(event.User, b.canQuitBotContextHandlerFunc(func(event *slack.MessageEvent) bool {
		...
		if i < 1 || i >= len(teams)+1 || err != nil {
			_, _, _ = b.slackBotAPI.PostMessage(event.Channel, slack.MsgOptionText("Wrong choice, try again or type 'quit'", false), AsUser)
			b.chooseTeam(event, username, teams, isSkipped)
			return false
		}
		// i-1 is related to above comments
		return b.chosenTeam(event, username, teams[i-1], isSkipped)
	}))

	return false
}

func (b *Bot) chosenTeam(event *slack.MessageEvent, username string, team string, isSkipped bool) bool {
	qs := b.scrum.GetQuestionSetsForTeam(team)

	if len(qs) == 0 {
		_, _, _ = b.slackBotAPI.PostMessage(event.Channel, slack.MsgOptionText("Your team has no questions defined.", false), AsUser)
		return false
	}

	if len(qs) == 1 {
		return b.chosenTeamAndContext(event, username, team, qs[0], isSkipped)
	}

	// get the question set (if more than one)
...
func (b *Bot) chooseContext(event *slack.MessageEvent, username string, team string, questionSets []*scrum.QuestionSet, isSkipped bool) bool {
	...
	b.setUserContext(event.User, b.canQuitBotContextHandlerFunc(func(event *slack.MessageEvent) bool {
		...
		return b.chosenTeamAndContext(event, username, team, questionSets[i], isSkipped)
	}))

	return false
}

func (b *Bot) chosenTeamAndContext(event *slack.MessageEvent, username string, team string, questionSet *scrum.QuestionSet, isSkipped bool) bool {
	if isSkipped {
...
func (b *Bot) answerQuestions(event *slack.MessageEvent, questionSet *scrum.QuestionSet, report *scrum.Report) bool {
	...
	if ans == len(questionSet.Questions) {
		...
		_, _, _ = b.slackBotAPI.PostMessage(event.Channel, slack.MsgOptionText("Thanks for your scrum report my :deer:! :bear: with us for the digest. :owl: see you later!\n If you want to start again just say `restart scrum`", false), AsUser)
		b.unsetUserContext(event.User)
		b.logger.WithFields(log.Fields{
			"user": report.User,
			"team": report.Team,
		}).Info("All questions answered, entry saved.")
		return false
	}
...
func (b *Bot) questionsOut(event *slack.MessageEvent, questionSet *scrum.QuestionSet, report *scrum.Report) bool {
	...
	_, _, _ = b.slackBotAPI.PostMessage(event.Channel, slack.MsgOptionText(question, false), AsUser)
